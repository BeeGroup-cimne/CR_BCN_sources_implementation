...
def gather_data(config, settings, args):
    for file in os.listdir('data/AMB_consumptions'):
        if file.endswith('.xlsx'):
            df = pd.read_excel(f"data/AMB_consumptions/{file}",
                               skiprows=2)  # todo: change way to get input

            save_AMB_consumption_data(
            ...
            save_AMB_consumption_data(data=df.to_dict(orient='records'), data_type="invoices",
                             row_keys=["CUPS", "Fecha inicio Docu. cálculo"],
                             column_map=[("info", "all")], config=config, settings=settings, args=args)


def save_AMB_consumption_data(data, data_type, row_keys, column_map, config, settings, args):
    if args.store == "kafka":
        try:
        ...
        try:
            ...
            utils.utils.log_string(f"Error saving AMB consumptions to HBASE: {e}")
    else:
        utils.utils.log_string(f"store {config['store']} is not supported")


def gather(arguments, config=None, settings=None):
    ap = argparse.ArgumentParser(description='Gathering data from AMB consumptions')
