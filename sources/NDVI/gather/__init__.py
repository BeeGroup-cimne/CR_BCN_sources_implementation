...
def gather_data(config, settings, args):
    for file in os.listdir('data/NDVI'):
        if file.endswith('.xlsx'):
            df = pd.read_excel(f"data/NDVI/{file}",
            ...
            save_ndvi_data(data=df.to_dict(orient='records'), data_type="invoices",
                             row_keys=["CUPS", "Fecha inicio Docu. cálculo"],
                             column_map=[("info", "all")], config=config, settings=settings, args=args)


def save_ndvi_data(data, data_type, row_keys, column_map, config, settings, args):
    if args.store == "kafka":
        try:
        ...
        try:
            ...
            utils.utils.log_string(f"Error saving NDVI to HBASE: {e}")
    else:
