...
def harmonize_postal_codes(data, **kwargs):
    config = utils.utils.read_config(settings.conf_file)
    # df_geometry = pd.DataFrame(data)
    morph_config = '\n[DataSource1]\nmappings:data/PostalCodes/mapping.yaml\nfile_path: {d_file}\n'


    data = json.load(open("data/PostalCodes/bcn_postal_codes.geojson"))

    with open("data/PostalCodes/data.json", "w") as d_file:
        json.dump({"postalCodes": {"geojson": data['features']}}, d_file)

    json_data = json.load(open("data/PostalCodes/data.json"))
    df_geometry = pd.json_normalize(json_data['postalCodes']['geojson'])

    df_geometry['geometry.coordinates'] = df_geometry['geometry.coordinates'].apply(
        lambda x: ' '.join([str(item) for item in x]))


    with open("sources/PostalCodes/harmonizer/temp.json", "w") as d_file:
        json.dump({"postalCodes": {"geojson": df_to_formatted_json(df_geometry, sep=".")}}, d_file)
