...
def gather_data(config, settings, args):
    for file in os.listdir('data/CensusTracts'):
        if file.endswith('.xlsx'):
            df = pd.read_excel(f"data/CensusTracts/{file}",
                               skiprows=2)  # todo: change way to get input

            save_census_tracts_data(
            ...
            save_census_tracts_data(data=df.to_dict(orient='records'), data_type="invoices",
                             row_keys=["CUPS", "Fecha inicio Docu. cálculo"],
                             column_map=[("info", "all")], config=config, settings=settings, args=args)


def save_census_tracts_data(data, data_type, row_keys, column_map, config, settings, args):
    if args.store == "kafka":
        try:
        ...
        try:
            ...
            utils.utils.log_string(f"Error saving census tracts supplies to HBASE: {e}")
    else:
