...
def gather_data(config, settings, args):
    for file in os.listdir('data/endesa'):
        if file.endswith('.xlsx'):
            df = pd.read_excel(f"data/endesa/{file}",
            ...
            save_endesa_data(data=df.to_dict(orient='records'), data_type="invoices",
                             row_keys=["CUPS", "Fecha inicio Docu. cálculo"],
                             column_map=[("info", "all")], config=config, settings=settings, args=args)


def save_endesa_data(data, data_type, row_keys, column_map, config, settings, args):
    if args.store == "kafka":
        try:
